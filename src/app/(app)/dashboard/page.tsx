...
export default async function DashboardPage({
  ...
  if (user.role !== "OWNER") {
    const columns = await getDocumentsSharedWithUser(user.organizationId, {
      id: user.id,
      role: user.role,
      teamId: user.teamId,
    });
    return <MemberDashboard name={user.name} role={user.role} columns={columns} />;
  }
  ...
  if (!activeTeam) {
    return (
      <div className="p-8">
        <h1 className="text-lg font-semibold text-foreground">Overall Dashboard</h1>
  ...
  const columns = await getTeamDocumentsByType(user.organizationId, activeTeam.id);
  ...
  return (
    <div className="p-8">
      ...
      <h2 className="mt-6 mb-3 text-sm font-medium text-foreground">
