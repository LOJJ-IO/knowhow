...
- **Desktop header** ([[FEAT-landing-header-nav]]): logo left; Log in + Talk to sales + Get Started right (shared `CTA_CLASS`, −20% then **+10%** → **~47.9px** height / matching type+padding; tight group). Sliding tabs removed. Mobile header still logo-only (+ bottom Get Started).
