...
- **Get Started handoff:** six-phase CTA — `idle → spinner (400ms) → blank (220ms) → arrows (280ms) → splitting → controls`. Pill pinches into two **~47.9px** circles via `liquid-gooey` on the deck rise/spread schedule. At `controls`, goo unmounts and **real** left/right circles (chevrons) drive the deck, each with the Get Started click sound (module-level `playClickSound`, one shared `AudioContext`). - **Get Started handoff:** six-phase CTA — `idle → spinner (400ms) → blank (220ms) → arrows (280ms) → splitting → controls`. Pill pinches into two **~43.5px** circles via `liquid-gooey` on the deck rise/spread schedule. At `controls`, goo unmounts and **real** left/right circles (chevrons) drive the deck, each with the Get Started click sound (module-level `playClickSound`, one shared `AudioContext`; gain **0.1** = −50% from prior 0.2). **Log In** / **Talk to Sales** also play that click. **Desktop:** looping **seven-card** feature ring (Unified Workspace → Auto-Own → Auto-Share → Oversight → DeepSearch → Org-Chart & Permissions → Instant Offboard); mats cycle green / blue / red / yellow; three visible + rest parked; ←/→ cascade unchanged ([[FEAT-landing-deck-carousel]]). **Mobile:** Cover Flow over the same seven — awaiting user decision on arrow semantics. The open deck only takes clicks on its cards + Cover Flow bar (`globals.css`); before that fix the full-screen deck layer swallowed every click on the arrows. **`data-open` flips with the split.**
  ...
  - **2026-09-12 (later):** desktop metaphor — main `{5.1, 8.9, 80.4×79.8%}`; Notes `{57.7, 63.5, 39×28%}` (= resize floor); folder `{86, 3}`; radii +5% (card 10.5px, folder plate 7.35px). See [[FEAT-landing-deck-notes-folder]].
...
- **Desktop footer links (2026-09-12):** **About Us** (left) + **Privacy Policy** (right) via `FooterStubLink` (`#about` / `#privacy`, `preventDefault` + click sound), Satoshi bold + underline + `cursor-pointer`, size `0.908552rem` (CTA face −20%), same row, `mt-[2px]` under “Take Control…”. Mobile mirrors. CTA pills also `cursor-pointer`. Click sound awaits `AudioContext.resume()`. Real destinations not wired yet.
