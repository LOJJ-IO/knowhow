...
- **Continue (revised 2026-09-17, user: "this button doesn't match the header" → centre text, drop the arrow circle, not full width):** now literally the header pill — `cn(CTA_CLASS, satoshi.className, "mt-3")` (`CTA_CLASS` carries no font; the header gets Satoshi from its row). Sizes to its label, left-aligned under the fields; computed styles verified identical to the header's Book a Demo pill. The first version (full-width, left label, white arrow circle) is gone, as is `ArrowRightIcon`.
