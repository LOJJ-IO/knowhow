...
- **Personal dashboard** ("Welcome back, {name}") for team leaders/members — Create New Work tiles (real Google Docs/Sheets/Slides/Forms "create new" URLs, opened in a new tab — these are universal Google URLs, not tied to the mocked integration), a static All Apps grid, and a "Shared With You" board using the same `TeamDocumentsBoard` component fed by `getDocumentsSharedWithUser`.
...
## Technical approach
`src/lib/queries.ts`: `getTeamDocumentsByType` (all docs for a team, both shared and not) vs. `getDocumentsSharedWithUser` (role-dependent visibility filter: OWNER sees `sharedWithOwner`, TEAM_LEADER sees `sharedWithLeader`, MEMBER sees either). Both return `TeamDocumentColumns` (`Prisma.DocumentGetPayload<{include:{owner:true}}>[]` per type).
